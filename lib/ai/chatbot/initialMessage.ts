...
export const getInitialMessage = async (
  ...
  console.log("Making initial message for player", player.id, "in world", world.id);
  ...
  try {
    ...
    console.log("Initial message generated successfully, text length:", result.text?.length || 0);
    return result;
  } catch (error) {
